...
day1_2Hz_Train = day1_2Hz_Train.trainningMode('normal');
[errMeasurement,checkpoint] = day1_2Hz_Train.trainKFold(trainParam,errMeasurement,checkpoint);
%%
for i = 1:100
tripNum = i;
thistrip = day1_2Hz_Train.LocalTripCell{tripNum}(:,[3 4]);
lat = thistrip(:,2);
lng = thistrip(:,1);
f1= figure(1);
plot(lat,lng)
...
%trainParam.in_attr = [3 4 7]; 
trainParam.in_attr = [3 4 ];
trainParam.tar_delay = 6:6;
%trainParam.tar_delay = 4:4;
trainParam.windowSize = 10:10;
trainParam.randomSeed = 2:2;
...
%%
[trainSet,testSet] = day1_2Hz_Train.genSegSet(trainParam);

%%
A = cell2mat(testSet.in_x);
B = cell2mat(testSet.in_y);
thistripID = 24;
A = testSet.in_x{thistripID};
B = testSet.in_y{thistripID};
len = length(B(:,1));
figure(1);
hold on;
for i = 1:1:len
...
xlabel('x-axis (m)');
ylabel('y-axis (m)');
title([num2str(len) ' segments generated from trip ID ' num2str(day1_2Hz_Train.getTripID('TripIndex',thistripID)) ' (2Hz,5s)']); 
%%
[Yt,Ytr,testerr] = day1_2Hz_Train.trainSeg(trainSet,testSet,trainParam);
%%
[trPredRe,trSetRe] = day1_2Hz_Train.retrieveGPSfromSeg(trainSet,Ytr,trainParam);
[tPredRe,tSetRe] = day1_2Hz_Train.retrieveGPSfromSeg(testSet,Yt,trainParam);
%%
idx = 20;
...
%[~,~,~]=day1_2Hz_Train.inspectTrip('TripIndex',tripIndex);
%%
...
subplot(2,4,[1 2 5 6]);
day1_2Hz_Train.inspectTrip('TripIndex',tripIndex,'addressOn',0,'getFigOnly','map');
hold off;
subplot(2,4,[3 4 7 8]);
