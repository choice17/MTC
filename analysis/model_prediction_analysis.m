...
%load ('../model/net-GPS-2017-08-28_145726.mat'); %lat/lng dt=1s nn=5 ws=5s 1Hz 
load ('../model/net-GPS-2017-10-23_221310.mat'); %lat/lng dt=1s nn=5 ws=5s 1Hz  


view(net);
day1_1Hz_Train = day1_1Hz_Train.setModel(net);
day1_1Hz_Train = day1_1Hz_Train.setTrainParam(trainParam);
day1_1Hz_Train = day1_1Hz_Train.trainningMode('UTM');

%% segmentation process observation

[trPredRe,trSetRe,trainSet,Ytr] = day1_1Hz_Train.modelPredict();
%% observe test set segment
    ...
    xlabel('x-axis (m)');
    ylabel('y-axis (m)');
    title([num2str(len) ' segments generated from trip  (1Hz,5s)']); 

%% prediction on all the trips in dataset

[trPredRe,trSetRe,trainSet,Ytr,day1_1Hz_Train] = day1_1Hz_Train.modelPredict();
...
day1_1Hz_Train.checkPredictError(option,idx)
